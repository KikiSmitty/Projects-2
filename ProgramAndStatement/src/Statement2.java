...
public class Statement2 extends StatementSecondary {
    ...
    public final void addToBlock(int pos, Statement s) {
        ...
        //create a new statement and cast to same dynamic type
        Statement2 localS = (Statement2) s;

        //create an empty sequence to hold the trees of the children
        Sequence<Tree<StatementLabel>> children = this.rep.newSequenceOfTree();

        //disassemble original statement
        StatementLabel root = this.rep.disassemble(children);

        //add the statement label to the correct position in the sequence of trees
    ...
    public final Statement removeFromBlock(int pos) {
        ...
        //disassemble the statement into the root and store it the children sequence
        StatementLabel root = this.rep.disassemble(children);

        //remove the desired children tree and store it in s
        s.rep = children.remove(pos);

        //reassemble using the store root and label
        this.rep.assemble(root, children);
    ...
    public final void assembleIfElse(Condition c, Statement s1, Statement s2) {
        ...
        //create two statements that cast
        Statement2 ifS = (Statement2) s1;
        Statement2 elseS = (Statement2) s2;

        //create new statement label with IF ELSE for kind and condition
        StatementLabel label = new StatementLabel(Kind.IF_ELSE, c);

        //create a sequence to store the children
        Sequence<Tree<StatementLabel>> children = this.rep.newSequenceOfTree();

        //add into respective positions
        children.add(0, ifS.rep);
        children.add(1, elseS.rep);
        this.rep.assemble(label, children);

        //clear if statement and else statement
        ifS.createNewRep();
        elseS.createNewRep();
    }
    ...
    public final Condition disassembleIfElse(Statement s1, Statement s2) {
        ...
        //create new sequence for the children
        Sequence<Tree<StatementLabel>> children = this.rep.newSequenceOfTree();
        StatementLabel label = this.rep.disassemble(children);
    ...
    public final String disassembleCall() {
        ...
        Sequence<Tree<StatementLabel>> children = this.rep.newSequenceOfTree();
        StatementLabel label = this.rep.disassemble(children);
        String instruction = label.instruction;
        this.createNewRep(); //clears this
        return instruction;
    }
